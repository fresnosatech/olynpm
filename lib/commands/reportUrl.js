...
function reportUrl() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        Object.keys(dependencies).forEach((packageName) => {
            ...
            if (!versionRange) {
                log.info({ log_prefix: constants_1.LOG_PREFIX }, `Skiped: ${packageName} version is missing`);
                return;
            }
            // version is latest
            if (/^latest$/.test(versionRange)) {
                list.push(packageName);
                log.info({ log_prefix: constants_1.LOG_PREFIX }, `Found: ${packageName}@${versionRange} -> ${versionRange}`);
                return;
            }
            // Skip package with invalid version range.
            if (!(0, valid_1.default)(versionRange) && !(0, semver_1.valid)(versionRange)) {
                log.info({ log_prefix: constants_1.LOG_PREFIX }, `Skiped: ${packageName}@${versionRange} version format not supported.`);
                return;
            }
            // Resolve range.
            const resolvedVersion = (0, exports.resolveVersionFromRange)(versionRange);
            log.info({ log_prefix: constants_1.LOG_PREFIX }, `Found: ${packageName}@${versionRange} -> ${resolvedVersion}`);
        ...
        console.log(`${constants_1.REPORT_URL}?packages=${pkgs}`, "\n\n");
    });
}
